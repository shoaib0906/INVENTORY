...
Class OutController extends Controller{
	...
	public function post_invoice(Request $request)
	{
		...
		foreach ($challan_nos as $challan_no ) {
			...
			->where('status','=',1)
			->where('challan_no','=',$challan_no->challan_no)
			->where('branch_id','=',Auth::user()->branch_id)
						->where('is_bill','=',0)
	...
	public function post_bill(Request $request)
	{
		...
		$customer_info = DB::table('customer_info')->where('branch_id','=',Auth::user()->branch_id)->get();
		//dd($customer_info);
		...
		if($challan_exist > 0)
		{
			...
			$product=DB::table('product_out')
				...
				DB::table('bill_register')->insert(['branch_id'=>Auth::user()->branch_id,
					'ref_no'=>$ref_no,'challan_no'=>$challan_no]);
		$product=DB::table('product_out')
    ...
    public function post_product_out(ProductoutRequest $request,Product_OUT $product_in)
	{
		...
		$is_billed = DB::table('product_out')->where('challan_no','=',$challan_no)->where('branch_id','=', Auth::user()->branch_id)->where('is_bill','!=',0)->count();
		...
		if($available>0){			
				...
				$product_in->category_out = $category_out;
