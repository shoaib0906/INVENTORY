...

{
   ...
   public function index(){
        ...
        $user_count = DB::table('Product_master')
                        ->where('status','=',1)
                        ->where('branch_id','=',Auth::user()->branch_id)->where('stock','>=',1)->count();
        //$dept_count=0;
        $dept_count = DB::table('Product_master')->where('branch_id','=',Auth::user()->branch_id)->where('status','=',1)->count();
        ...
        $present_count =  DB::table('bill_tran')->where('branch_id','=',Auth::user()->branch_id)->count();
        $expire_count  = DB::table('bill_tran')->where('branch_id','=',Auth::user()->branch_id)->sum('net_amt');
        $discount = DB::table('bill_tran')->where('branch_id','=',Auth::user()->branch_id)->sum(DB::raw('total_amt*dis_percent/100'));
        $less_amt = DB::table('bill_tran')->where('branch_id','=',Auth::user()->branch_id)->sum('less_amt');
